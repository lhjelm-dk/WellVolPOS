...
# ------------------------------------------------------- the two partitions
def test_the_two_partitions_cut_the_same_closure_in_different_places(reduced):
    """C5's whole content is that the cut moves, so the cut is what gets asserted.

    Rose cuts at the **well** and this app at **LKH = min(contact, exit)**, so with an
    exit below the entry the app's cut is deeper and its proven part is larger by
    exactly the penetrated slice. Getting this backwards is the confusion the figure
    was built to end, and both panels look plausible either way round.
    """
    import numpy as np
    import wellvolpos.viz.interactive as I

    ad = AreaDepth.from_trials(reduced.col("contact"), reduced.col("area"))
    entry, exit_, contact = 3500.0, 3550.0, 3600.0
    fig = I.pfig_c5_partitions(ad, z_entry=entry, z_exit=exit_, z_contact=contact)

    names = [t.name for t in fig.data if t.name]
    assert names == ["Rose updip", "Rose downdip", "Proven", "Unproven below LKH"]

    def span(name):
        t = next(t for t in fig.data if t.name == name)
        y = np.asarray(t.y, dtype=float)
        return float(y.min()), float(y.max())

    # Rose's cut is the entry; ours is the exit, because the exit is above the contact.
    assert span("Rose updip")[1] == pytest.approx(entry, abs=1.0)
    assert span("Proven")[1] == pytest.approx(min(contact, exit_), abs=1.0)
    # So the app's proven reaches deeper than Rose's updip -- by the penetrated slice.
    assert span("Proven")[1] > span("Rose updip")[1]

    # Both partitions cover the same closure, crest to contact.
    for a, b in (("Rose updip", "Rose downdip"), ("Proven", "Unproven below LKH")):
        assert span(a)[0] == pytest.approx(ad.shallowest, abs=1.0)
        assert span(b)[1] == pytest.approx(contact, abs=1.0)


def test_c5_is_two_panels_on_one_depth_axis_and_not_a_twinned_one(reduced):
    """A second *panel* has a yaxis2 legitimately; a twinned axis is what is banned."""
    import wellvolpos.viz.interactive as I

    ad = AreaDepth.from_trials(reduced.col("contact"), reduced.col("area"))
    fig = I.pfig_c5_partitions(ad, z_entry=3500.0, z_exit=3550.0, z_contact=3600.0)
    ys = [v for k, v in fig.layout.to_plotly_json().items() if k.startswith("yaxis")]
    assert len(ys) >= 2, "expected two panels"
    assert not any(y.get("overlaying") for y in ys), "no axis may be twinned"


def test_c5_has_an_export_twin():
    import wellvolpos.viz.figures as F
    import wellvolpos.viz.interactive as I

    assert hasattr(F, "fig_c5_partitions") and hasattr(I, "pfig_c5_partitions")
